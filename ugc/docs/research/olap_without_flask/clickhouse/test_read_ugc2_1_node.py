...
async def prepair_database(client: ChClient) -> None:
    assert await client.is_alive()
    await client.execute('DROP DATABASE IF EXISTS benchmarks;')
    await client.execute('CREATE DATABASE benchmarks;')
